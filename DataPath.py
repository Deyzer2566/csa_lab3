...
class DataPath:
	...
	def __init__(self):
		...
		self.registers_memory_alu_or_flags_MUX = Registers_Memory_ALU_or_Flags_MUX.MEMORY
	...
	def set_AR(self):
		match self.registers_memory_alu_or_flags_MUX:
			...
			case Registers_Memory_ALU_or_Flags_MUX.FLAGS:
				...
				self.DR = self.interruption_num | int(self.is_in_interruption)<<5 | int(NZC[NZC_Flags.CARRY.value])<<6 | int(NZC[NZC_Flags.ZERO.value])<<7 | int(NZC[NZC_Flags.NEGATIVE.value])<<8
		if self.AR & self.memory_mapped_registers_address != self.memory_mapped_registers_address:
			self.data_from_memory = self.memory[self.AR]<<24 | self.memory[self.AR+1]<<16 | self.memory[self.AR+2]<<8 | self.memory[self.AR+3]
		else:
	...
	def reset_DR(self):
		self.DR = 0
		self.update_ALU_out()
	def select_registers_memory_or_alu(self, memory_or_alu:Registers_Memory_ALU_or_Flags_MUX):
		...
		match self.ALU_operation:
			...
			case ALU_Operation.SUM:
				...
				if (self.useCarry and self.NZC[2]) or self.plus1:
					out += 1
				self.ALU_out = [crop(out,32), out&(1<<31) != 0, crop(out,32)==0, out > 2**32-1]
			case ALU_Operation.LS:
				out = first << second
				self.ALU_out = [crop(out,32), out&(1<<31) != 0, crop(out,32)==0, out & (2**32) != 0]
			case ALU_Operation.RS:
				out = first >> second
				self.ALU_out = [crop(out,32), out&(1<<31) != 0, crop(out,32)==0, (first >> (second-1)) & 1 != 0]
			...
			case ALU_Operation.MUL:
				out = first * second
				self.ALU_out = [crop(out,32), out&(1<<31) != 0, crop(out,32)==0, False]
			...
			case ALU_Operation.SUB:
				out = first - second
				self.ALU_out = [crop(out, 32), out & (1<<31) != 0, crop(out,32)==0, (first + second) > (1<<32)]
	def set_NZC(self):
		match self.alu_or_flags:
			case ALU_or_flags_MUX.ALU:
				self.NZC = [self.ALU_out[0] & (1<<8) != 0, self.ALU_out[0] & (1<<7) != 0, self.ALU_out[0] & (1<<6) != 0]
